...
let indexedDB=window.indexedDB||window.mozIndexedDB||window.webkitIndexedDB||window.msIndexedDB||window.shimIndexedDB;let open=indexedDB.open("Restaurants",2);let restaurantList=[];// Create the schema
open.onupgradeneeded=function(){let db=open.result;let store=db.createObjectStore("RestaurantStore",{keyPath:"id"});db.createObjectStore("OfflineReviews",{keyPath:"id",autoIncrement:true});let allReviews=db.createObjectStore("AllReviews",{keyPath:"id"});allReviews.createIndex("restaurant_id","restaurant_id",{unique:false});let changedFav=db.createObjectStore("ChangedFavs",{keyPath:"id",autoIncrement:true});var customerObjectStore;console.log('idb upgrade needed');fetch(DBHelper.DATABASE_URL).then(function(response){return response.json();}).then(function(restaurants){store.transaction.oncomplete=function(){customerObjectStore=db.transaction("RestaurantStore","readwrite").objectStore("RestaurantStore");restaurants.forEach(function(customer){customerObjectStore.add(customer);});};}).catch(function(){const error=`Request failed.`;callback(error,null);});};open.onsuccess=function(){// Start a new transaction
var db=open.result;var tx=db.transaction("RestaurantStore","readwrite");var or=db.transaction("OfflineReviews","readwrite");var store=tx.objectStore("RestaurantStore");var offlineReviews=or.objectStore("OfflineReviews");console.log('idb success');offlineReviews.openCursor().onsuccess=function(event){var cursor=event.target.result;if(cursor){postReview(true,cursor.value);cursor.continue();}else{callback(null,restaurantList);console.log('list',restaurantList);console.log('Entries all displayed.');offlineReviews.clear();}};store.openCursor().onsuccess=function(event){var cursor=event.target.result;if(cursor){restaurantList.push(cursor.value);cursor.continue();}else{callback(null,restaurantList);console.log('list',restaurantList);console.log('Entries all displayed.');}};// Close the db when the transaction is done
