 ...
 */fetchCusineAndNeighborhood=()=>{fetch(restaurantApi).then(function(response){return response.json();}).then(function(myJson){console.log(myJson);fillNeighborhoodsHTML(myJson);fillCuisinesHTML(myJson);openDb(myJson);});};/**
 * Fetch all neighborhoods and set their HTML.
 ...
 */createRestaurantHTML=restaurant=>{const li=document.createElement('li');const image=document.createElement('img');image.className='restaurant-img';image.setAttribute('alt','Picture of '+restaurant.name);image.src='/img/'+restaurant.photograph+'.jpg';li.append(image);const content=document.createElement('div');content.setAttribute('class','restaurant-summary');const name=document.createElement('h3');name.innerHTML=restaurant.name;content.append(name);const neighborhood=document.createElement('p');neighborhood.innerHTML=restaurant.neighborhood;content.append(neighborhood);const address=document.createElement('p');address.innerHTML=restaurant.address;content.append(address);const more=document.createElement('a');const moreText=`View Details <span class="visually-hidden">about ${restaurant.name}</span>`;more.innerHTML=moreText;more.href=DBHelper.urlForRestaurant(restaurant);content.append(more);li.append(content);return li;};/**
 * Add markers for current restaurants to the map.
 */addMarkersToMap=(restaurants=self.restaurants)=>{restaurants.forEach(restaurant=>{// Add marker to the map
const marker=DBHelper.mapMarkerForRestaurant(restaurant,self.map);google.maps.event.addListener(marker,'click',()=>{window.location.href=marker.url;});self.markers.push(marker);});};openDb=restaurants=>{const dbName="RestaurantList";var request=indexedDB.open(dbName,3);var customerObjectStore;request.onerror=function(event){// Handle errors.
};request.onupgradeneeded=function(event){var db=event.target.result;var res=restaurants;console.log('res',res);// Create an objectStore to hold information about our customers. We're
// going to use "ssn" as our key path because it's guaranteed to be
// unique - or at least that's what I was told during the kickoff meeting.
var objectStore=db.createObjectStore("restaurants",{keyPath:"id"});// Create an index to search customers by name. We may have duplicates
// so we can't use a unique index.
objectStore.createIndex("name","name",{unique:false});// Use transaction oncomplete to make sure the objectStore creation is
// finished before adding data into it.
objectStore.transaction.oncomplete=function(event){// Store values in the newly created objectStore.
customerObjectStore=db.transaction("restaurants","readwrite").objectStore("restaurants");restaurants.forEach(function(customer){customerObjectStore.add(customer);});console.log('res',res);};};};